...
async function xdua ({ APP_TOKEN }) {
    ...
    async function initialize() {
        try{
            let localUserToken = store.get('usr_token');
            //--对付网页刷新-开始
    ...
    * */
    ...
    async function login({ by, ustr, pwd, shop, role }) {
        try {
            ...
            let resp = e.response
            let ret = {
                apid: resp.headers["x-ca-request-id"],
                status:resp.status,
                error: resp.headers["x-ca-error-code"], 
                reason: resp.statusText+":"+resp.headers["x-ca-error-message"],
            }
            return ret 
        }
    }
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addUser({ by,extra,ustr, pwd, vfcode, shop, role,name,sex,bday,avatar,tabx,api_action="adduser"}) {
        try {
            ...
            if (_.isNil(bday)) {bday = ""}
            if (_.isNil(sex)) {sex = "u"}
            if (_.isNil(avatar)) {avatar = ""}
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function putUser(user_id,tabx,update,api_action="setuser") {
        try {
            const API_PATH = '/user/' + user_id
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const localToken = getLocalToken()
    ...
    async function getUser({user_id,tabx,api_action,fields}) {
        try {
            ...
            const API_PATH = '/user/' + user_id+"?action="+api_action+"&tabx="+tabx+"&fields="+fields
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            console.log(url);
            const localToken = getLocalToken()
            const headers = {
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addUgrp({ name, brief, avatar }) {
        try {
            const API_PATH = '/zone'
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const localToken = getLocalToken()
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addRule({ code, name,shop_id,extra,avatar}) {
        try {
            const API_PATH = '/rule'
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const localToken = getLocalToken()
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addUaff({ user_id, zone_id}) {
        try {
            const API_PATH = '/uaff'
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const localToken = getLocalToken()
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addCorp({ name,code,api_action="addcorp"}) {
        try {
            if ( api_action != "addcorp") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/corp?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addShop({ name,code,api_action="addshop"}) {
        try {
            if ( api_action != "addshop") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/shop?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addBuild({ vila_id,type,name,note,api_action="addbuild"}) {
        try {
            if ( api_action != "addbuild") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/build?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addTier({ build_id,type,name,note,api_action="addtier"}) {
        try {
            if ( api_action != "addtier") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/tier?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addRoom({ tier_id,type,name,note,api_action="addroom"}) {
        try {
            if ( api_action != "addroom") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/room?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addSlot({ room_id,type,name,note,api_action="addslot"}) {
        try {
            if ( api_action != "addslot") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/slot?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addIdcard({ name,addr,id,issuer,nation,expire,validate,api_action="addidcard"}) {
        try {
            if ( api_action != "addidcard") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/idcard?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addElder({ name,certid,api_action="addelder"}) {
        try {
            if ( api_action != "addelder") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/elder?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addMats({ devp_id,snid,api_action="addmats"}) {
        try {
            if ( api_action != "addmats") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/mats?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addSleep({ slot_id,mats_id,elder_id,api_action="addsleep"}) {
        try {
            if ( api_action != "addsleep") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/sleep?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addEldla({ elder_id,name,certid,relation,api_action="addeldla"}) {
        try {
            if ( api_action != "addeldla") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/eldla?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addEldiag({ disease_name,elder_id,diag_date,api_action="addeldiag"}) {
        try {
            if ( api_action != "addeldiag") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/eldiag?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
    ...
    async function addElderep({ 
        ...
        try {
            ...
            let api_token = getLocalUsrToken();
            const API_PATH = '/elderep?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addEyegrp({ name,grpid,api_action="addeyegrp"}) {
        try {
            if ( api_action != "addeyegrp") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/eyegrp?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function qryEyegrp(param) {
        try {
            let api_path = '/eyegrp'
            if (JSON.stringify(param) !== '{}') {
                api_path = '/eyegrp?'
    ...
    async function getEyegrp({eyegrp_id}) {
        try {
            const API_PATH = '/eyegrp/' + eyegrp_id
                const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
                const localToken = getLocalToken()
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addEyelog({ Event,Time,DeviceId,DeviceStatus,StreamId,StreamStatus,GroupId,GBId,api_action="addeyelog"}) {
        try {
            if ( api_action != "addeyelog") {
                throw new ArgumentError('Invalid Api Action')
            }
            let api_token = getLocalUsrToken();
            const API_PATH = '/eyelog?action='+api_action
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
