...
const OSS_INFO_AVATAR = {
    url: 'http://api.xdua.com/sts/osser',
    region: 'oss-cn-beijing',
    bucket: 'avatar-xdua-com'
}

async function getAvatarClient(){
  try{
    let raw_res = await fetch(OSS_INFO_AVATAR.url)
    let res = await raw_res.json()
    let client = new OSS({
...
async function addavatar(file,ossfilepath,options) {
  try{
    let oss = await getAvatarClient()
