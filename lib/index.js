    ...
    * */
    ...
    async function isLogin() {
        try {
            ...
            const url = API_END_POINT + '/login/' + token
            const sign = generateSign({ method: 'GET', path: '/login/'+token, appSecret: getAppSecret(), appKey: getAppKey() })
            const headers = {
                accept: APPLICATION_JSON,
                'content-type': APPLICATION_X_WWW_FORM_URLENCODED,
                Authorization: sign
     ...
     * @returns {Object<*>} the result of the login. If failed, return the reason of the failure
     */
    async function addZone({ name, brief, avatar ,pid}) {
        try {
            const API_PATH = '/zone'
            const url = API_END_POINT + API_PATH // Add '+86-' to the username, since we currently only support registration from China mainland
            const localToken = getLocalToken()
